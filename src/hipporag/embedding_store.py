...
class EmbeddingStore:
    def __init__(self, embedding_model, db_filename, batch_size, namespace):
        ...
        self._embedding_matrix_cache: Dict[Tuple[str, str], np.ndarray] = {}
        self._load_data()
    ...
    def _get_embedding_matrix(
        ...
        cache_key = (normalized_dtype.str, normalized_dtype.name)
        cached = self._embedding_matrix_cache.get(cache_key)
        if cached is None:
            cached = np.asarray(self.embeddings, dtype=normalized_dtype)
            self._embedding_matrix_cache[cache_key] = cached
        return cached

    def get_embedding(self, hash_id, dtype=np.float32) -> np.ndarray:
        return self._get_embedding_matrix(dtype=dtype)[self.hash_id_to_idx[hash_id]]
    ...
    def get_embeddings(self, hash_ids, dtype=np.float32) -> np.ndarray:
        ...
        embeddings = self._get_embedding_matrix(dtype=dtype)[indices]
