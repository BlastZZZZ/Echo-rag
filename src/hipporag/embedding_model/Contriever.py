...
from .base import BaseEmbeddingModel, EmbeddingConfig, prefix_queries_with_instruction
...
class ContrieverModel(BaseEmbeddingModel):
    ...
    def batch_encode(self, texts: List[str], **kwargs) -> None:
        ...
        texts_to_encode = prefix_queries_with_instruction(list(texts), params.get("instruction", ""))

        batch_size = params.pop("batch_size", 16)

        logger.debug(f"Calling {self.__class__.__name__} with:\n{params}")
        if len(texts_to_encode) <= batch_size:
            results = self.encode(texts_to_encode)
        else:
            pbar = tqdm(total=len(texts_to_encode), desc="Batch Encoding")
            results = []
            for i in range(0, len(texts_to_encode), batch_size):
                results.append(self.encode(texts_to_encode[i:i + batch_size]))
