...
class DSPyFilter:
    def __init__(self, hipporag):
        ...
        self.fail_on_exception = bool(getattr(hipporag.global_config, "rerank_filter_fail_on_exception", False))
        self.filter_mode = str(getattr(hipporag.global_config, "rerank_filter_mode", "dspy"))
    ...
    def llm_call(self, question, fact_before_filter):
        ...
        if len(response) > 1:
            if isinstance(response[1], dict):
                self.last_call_metadata = response[1]
            return response[0]
        return response
    ...
    def rerank(self,
        ...
        if self.filter_mode == "passthrough":
            fallback_n = len_after_rerank if len_after_rerank is not None else len(candidate_indices)
            return (
                candidate_indices[:fallback_n],
                candidate_items[:fallback_n],
                {'confidence': None, 'fallback': 'candidate_order_passthrough'},
            )
        try:
        ...
        for generated_fact in generated_facts:
            if isinstance(generated_fact, list):
                generated_fact = tuple(generated_fact)
            if generated_fact in candidate_items:
                result_indices.append(candidate_items.index(generated_fact))
                continue
        ...
        if not result_indices:
            ...
            return (
                ...
                {
                    'confidence': None,
                    'fallback': 'candidate_order_when_filter_empty',
        ...
        return (
            ...
            {'confidence': None, 'metadata': dict(self.last_call_metadata)},
        )
