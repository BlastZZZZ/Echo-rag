...
def _extract_response_field(real_response: str, field_name: str):
...
def _normalize_openie_atom(value):
    ...
    if isinstance(value, str):
        ...
        return None
    if not normalized or normalized in {"...", "Ellipsis"}:
        return None
    return normalized


def _sanitize_named_entities(named_entities) -> List[str]:
    if not isinstance(named_entities, list):
        return []
    sanitized: List[str] = []
    seen = set()
    for entity in named_entities:
        normalized = _normalize_openie_atom(entity)
...
def _sanitize_triples(triples) -> List[List[str]]:
    ...
    for triple in triples:
        if not isinstance(triple, (list, tuple)) or len(triple) != 3:
            continue
        normalized_triple: List[str] = []
        for item in triple:
            normalized = _normalize_openie_atom(item)
            if normalized is None:
                normalized_triple = []
                break
            normalized_triple.append(normalized)
        if len(normalized_triple) == 3:
            sanitized.append(normalized_triple)
    return sanitized


def _extract_ner_from_response(real_response):
    return _sanitize_named_entities(_extract_response_field(real_response, "named_entities"))
...
class OpenIE:
    ...
    def ner(self, chunk_key: str, passage: str) -> NerRawOutput:
        ...
        try:
            ...
            unique_entities = _extract_ner_from_response(real_response)
    ...
    def triple_extraction(self, chunk_key: str, passage: str, named_entities: List[str]) -> TripleRawOutput:
        def _extract_triples_from_response(real_response):
            return _sanitize_triples(_extract_response_field(real_response, "triples"))
